...
export interface GramJsClientConfig {
...
/**
 * Real GramJS (MTProto) implementation of ITelegramUserTransport.
 * Connects to Telegram using a session string from PlatformConnection.
 *
 * Unit tests use FakeTelegramUserTransport instead — this class requires a live session.
 */
export class GramJsClient implements ITelegramUserTransport {
  private readonly client: TelegramClient
  private readonly logger: Logger

  constructor({ sessionString, apiId, apiHash, logger }: GramJsClientConfig) {
    this.logger = logger.child({ component: 'GramJsClient' })
  ...
  private extractMessageFromUpdates(updates: Api.TypeUpdates): Api.Message {
    ...
    throw new ConnectorError('Could not extract message from updates', 'TRANSPORT_ERROR')
  }
}
