...
export function createTelegramBotPool(deps: {
  prisma: PrismaClient
  config: Config
  logger: Logger
  baseDir: string
  ...
  return {
    platform: 'telegram',
    type: 'bot',
    workerScript: join(baseDir, '../../../packages/telegram-bot-connector/src/worker.ts'),
    ...
    logger: logger.child({ pool: 'telegram:bot' }),
